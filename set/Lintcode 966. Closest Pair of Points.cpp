...
public:
    ...
    double getClosestDistance(vector<double> &x, vector<double> &y) {
        ...
        auto dist = [](const PDD& $1, const PDD& $2) -> double {
            double x1, x2, y1, y2;
            tie(x1, y1) = $1;
            tie(x2, y2) = $2;
        ...
        for (int l = 0, r = 0; r < n; r++) {
            ...
            for (auto it = cur.lower_bound({all[r].first, all[r].second - ans});
                it != cur.end() && it->second - all[r].second < ans; it++) {
                
                const double d = dist(*it, all[r]);
                ans = min(ans, d);
            }
            cur.insert(all[r]);
        }
